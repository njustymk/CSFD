...
def calculate_ap_res(bbox_gt_dir, result_dir, ovthresh):
    ...
    for index, sample_id in tqdm(enumerate(sample_ids)):
        ...
        result_file = os.path.join(result_dir, sample_id+'_ir.txt')
...
if __name__ == '__main__':
    data_split = 'csf_all'
    res_splits = ['csf_all_dsfd', 'csf_all_faceboxes', 'csf_all_pyramidbox', 'csf_all_s3fd', 'csf_all_tinyface']
    for res_split in res_splits:

        data_root = 'data/test/{}'.format(data_split)
        print('data_root', data_root)
        bbox_ir_gt_dir =  osp.join(data_root, 'bbox_ir_gt')

        result_dir = 'results_detectors/{}'.format(res_split)
        print(result_dir)
